...
- Bead 34 main redeploy validation passed: `make verify-all`, `git diff --check`, `npm --prefix web audit --audit-level=high`, Wrangler dry run, production deploy, live homepage/blog/docs/robots/sitemap host smokes, `www` and workers.dev health smokes, and unauthenticated `/mcp` 401 challenge check.
