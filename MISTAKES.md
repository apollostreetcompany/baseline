...
- 2026-06-12: Oops: Cloudflare Worker code uploads can silently drop Worker-only secrets when metadata/binding inheritance is wrong. Version uploads that read back with missing bindings, `cf workers scripts content update`, and `wrangler deploy --keep-vars` all produced versions with only `APP_URL` in this session; live health caught the regression and rollback restored version `0ddb077d-5188-4256-98eb-baf449a30d4c`. Never deploy a new Worker version unless readback shows the full binding set or the secret values are intentionally rehydrated from a safe secret manager.
