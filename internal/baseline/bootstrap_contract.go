...
func renderBootstrapContract(cfg Config) string {
	...
	b.WriteString("- `baseline rerun RUN_ID`: repair known guardrails and rerun a failed lifecycle run with the same pack selection.\n")
	b.WriteString("- `baseline repair openclaw`: apply OpenClaw-specific local guardrails without starting an eval.\n")
	...
	b.WriteString("Exit codes matter: `baseline report RUN_ID --json` exits `0` for completed, `2` for still running, and `1` for failed lifecycle runs. Agents should use those codes instead of treating any JSON response as success.\n\n")
	...
	b.WriteString("- Disappeared background run: if a run says `pid ... is no longer running and no result row was written`, read stdout/stderr for the last progress line, run `baseline repair openclaw` for OpenClaw targets, then ask the operator before `baseline rerun RUN_ID`.\n")
